...
import { formatDateForPdf, generatePdfReport } from "../utils/pdf/commonPdfGenerator";
...
async function exportAttendancePdf(title, subtitle, attendanceData = []) {
  ...
  generatePdfReport({
    ...
    subtitle: `${subtitle || "-"} | Generated: ${formatDateForPdf(new Date())}`,
    reportType: "attendance_report",
    headerMetaLeft: subtitle || "-",
    headerMetaRight: `Generated: ${formatDateForPdf(new Date())}`,
    summaryCards: [
      { label: "TOTAL ROWS", value: String(rows.length) },
      { label: "PRESENT", value: String(rows.filter((r) => r[3] === "Present").length) },
      { label: "ABSENT", value: String(rows.filter((r) => r[3] === "Absent").length) },
      { label: "LEAVE / HALF DAY", value: String(rows.filter((r) => r[3] === "Leave" || r[3] === "Half Day").length) },
    ],
    columns: ["Date", "Name", "Role", "Status", "Note"],
    rows,
